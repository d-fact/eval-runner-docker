...
public class BytecodeUtils {
	...
	public static String getShortMethodName(final String vmclassname, final String vmmethodname,
											final String vmdesc) {
	...
	public static boolean matchWithGenericType(final String sig1,
		...
		if (isMethodName(sigcp1) && isMethodName(sigcp2)) {
			...
			int numOfPar1 = Math.min(par1.split(",").length, par1.replaceAll("[)|(|\\s+]", "").length());
			int numOfPar2 = Math.min(par2.split(",").length, par2.replaceAll("[)|(|\\s+]", "").length());
			return numOfPar1 == numOfPar2;
		}
	...
	public static List<String> getArgsAsString(final String vmclassname, final String vmdesc, boolean qualified) {
		...
		for (final Type arg : arguments) {
			...
			if (qualified) {
				args.add(getClassName(arg.getClassName()));
			} else {
				args.add(getShortTypeName(arg));
			}
        }
		return args;
	}

	public static String getRetType(final String vmdesc) {
		Type t =  Type.getReturnType(vmdesc);
		return getClassName(t.getClassName());
	}
