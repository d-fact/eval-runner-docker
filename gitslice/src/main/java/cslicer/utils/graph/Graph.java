...
public class Graph<T> implements Serializable {
	...
	/**
	 * Output graph to a file in Datalog (Souffle dialect), for storing hunk deps
	 *
	 * @param factsFilePath path of output datalog facts file
	 */
	public void toDatalog(String factsFilePath) {
		StringBuilder factsContents = new StringBuilder() ;
		for (Edge<T> e : edges) {
			Vertex<T> fromVertex = e.getFrom();
			Vertex<T> toVertex = e.getTo();
			String row = String.format("%s\t%s\n", fromVertex.getName(), toVertex.getName());
			factsContents.append(row);
		}
		try {
			File taFile = new File(factsFilePath);
			FileWriter fWriter = new FileWriter(taFile, false);
			fWriter.write(factsContents.toString());
			fWriter.flush();
			fWriter.close();
		} catch(IOException e) {
			e.printStackTrace();
