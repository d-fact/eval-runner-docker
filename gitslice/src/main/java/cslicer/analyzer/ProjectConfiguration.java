...
public class ProjectConfiguration {
	...
	// for deps facts util
	private String fDepsFactsFilePath = null;
	private String fMD5ToPlainStringMapFilePath = null;
	private String fDepsPlainFilePath = null;
	...
	public ProjectConfiguration(Path configPath) {
		...
		try {
			...
			this.setRepositoryPath(
					handleHomeTilde(config.getProperty("repoPath", DEFAULT_REPO_PATH)));
			this.setJacocoExecPath(
					handleHomeTilde(config.getProperty("execPath", DEFAULT_JACOCO_PATH)));
			...
			this.setSourceRootPath(
					handleHomeTilde(config.getProperty("sourceRoot", null)));
			this.setClassRootPath(
					handleHomeTilde(config.getProperty("classRoot", DEFAULT_CLASS_ROOT)));
			this.setTestClassRootPath(
					handleHomeTilde(config.getProperty("testClassRoot", DEFAULT_TEST_CLASS_ROOT)));
			this.setBuildScriptPath(
					handleHomeTilde(config.getProperty("buildScriptPath", DEFAULT_BUILD_PATH)));
			...
			this.setTouchSetPath(
					handleHomeTilde(config.getProperty("touchSetPath", DEFAULT_TOUCH_SET_PATH)));
			...
			this.setDepsFactsFilePath(config.getProperty("depsFactsFile", null));
			this.setMD5ToPlainStringMapFilePath(config.getProperty("MD5ToPlainStringMapFile", null));
			this.setDepsPlainFilePath(config.getProperty("depsPlainFile", null));
	...
	public boolean isConsistent() {
		...
		if (fRepoPath == null || (fStartId == null && fHistoryLength < 0)) {
			PrintUtils.print("RepoPath is not provided or history configuration is incorrect.",TAG.WARNING);
			return false;
		}
		...
		if (fBuildPath == null) {
			PrintUtils.print("BuildPath (buildScriptPath) is not provided.", TAG.WARNING);
			return false;
		}
		...
		if ((fTestJacocoExecPath == null) != (fSourceRootPath.size() == 0)
			...
			PrintUtils.print("Configuration of ClassRoot/SourceRoot/JacocoExec is incorrect", TAG.WARNING);
			return false;
		}
	...
	public String getDepsFactsFilePath() { return fDepsFactsFilePath; }

	public ProjectConfiguration setDepsFactsFilePath(String p) {
		this.fDepsFactsFilePath = p;
		return this;
	}

	public String getMD5ToPlainStringMapFilePath() { return fMD5ToPlainStringMapFilePath; }

	public ProjectConfiguration setMD5ToPlainStringMapFilePath(String p) {
		this.fMD5ToPlainStringMapFilePath = p;
		return this;
	}

	public String getfDepsPlainFilePath() { return fDepsPlainFilePath; }

	public ProjectConfiguration setDepsPlainFilePath(String p) {
		this.fDepsPlainFilePath = p;
		return this;
	}
	...
	private String handleHomeTilde(String path) {
		if(path!=null) {
