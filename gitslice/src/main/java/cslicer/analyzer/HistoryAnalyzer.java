...
public class HistoryAnalyzer {
	...
	public enum FactFormat {TA, SOUFFLE}
	...
	public HistoryAnalyzer(ProjectConfiguration config)
		...
		try {
			...
			PrintUtils.print(String.format("%s exists, files inside it might be overridden.", fOutputPath),
	...
	protected List<RevCommit> computeHunkDepSet(
	    ...
	    return computeHunkDepSet(cSet, aSet, cache, null);
	}

	protected List<RevCommit> computeHunkDepSet(
			final Collection<RevCommit> cSet, Collection<RevCommit> aSet,
			DependencyCache cache, FactFormat outputFormat) {
		int i = 0;
		...
		if (fConfig.getOutputHunkGraph()) {
			cache.outputCacheToFile(
					FileUtils
							...
							.getAbsolutePath());
			// String hunkFactsFile = Paths.get(fOutputPath, "50-hunkdep.ta").toString();
			// cache.outputCacheToFacts(hunkFactsFile);
			outputHunkToFacts(cache, outputFormat);
		}
	...
	protected void outputHunkToFacts(DependencyCache cache, FactFormat outputFormat) {
		switch (outputFormat) {
			case SOUFFLE:
				String hunkFactsFile = Paths.get(fOutputPath, "50-hunkdep.facts").toString();
				cache.outputCacheToDatalog(hunkFactsFile);
				break;
			case TA:
				hunkFactsFile = Paths.get(fOutputPath, "50-hunkdep.ta").toString();
				cache.outputCacheToTA(hunkFactsFile);
				break;
			default:
				break;
		}
