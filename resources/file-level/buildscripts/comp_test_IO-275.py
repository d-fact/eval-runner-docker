...
if __name__ == '__main__':
    ...
    if option == 'compile':
        ...
        ret = sub.call ([mvn, 'org.apache.maven.plugins:maven-surefire-plugin:2.20:test', '-Dtest=FileUtilsTestCase#testContentEqualsIgnoreEOL'])
        print ret
        if ret == 1:
            sys.exit(1)
        ret = sub.call ([mvn, 'org.apache.maven.plugins:maven-surefire-plugin:2.20:test', '-Dtest=IOUtilsTestCase#testContentEqualsIgnoreEOL'])
        print ret
        if ret == 0:
            sys.exit(0)
        else:
