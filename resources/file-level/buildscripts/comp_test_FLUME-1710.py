...
if __name__ == '__main__':
    ...
    if option == 'compile':
        ...
        if not os.path.isdir(repopath + '/flume-ng-sdk/target/test-classes'):
            print 'copy test classes to repo!'
            shutil.copytree(TEST_CLASSES_BACKUP_DIR + '/test-classes', \
                    repopath + '/flume-ng-sdk/target/test-classes')
        ret = sub.call ([mvn, 'org.apache.maven.plugins:maven-surefire-plugin:2.20:test', '-Dtest=TestEventBuilder#testJsonEventUnsupportedEncoding'])
        print ret
        if ret == 0:
            sys.exit(0)
        else:
