...
public class WalaFactWriter extends JavaFactWriter {
    ...
    String writeMethod(IMethod m) {
        ...
        _db.add(STRING_RAW, null, result, result);
        _db.add(METHOD, null, result, WalaRepresentation.simpleName(m.getReference()), WalaRepresentation.params(m.getReference()), writeType(m.getReference().getDeclaringClass()), writeType(m.getReturnType()), m.getDescriptor().toUnicodeString(), arity);
        for (Annotation annotation : m.getAnnotations()) {
            _db.add(METHOD_ANNOTATION, null, result, fixTypeString(annotation.getType().toString()));
...
//        }
    ...
    void writeClassOrInterfaceType(IClass c) {
        ...
        if (c.isInterface()) {
            _db.add(INTERFACE_TYPE, null, classStr);
        }
        else {
            _db.add(CLASS_TYPE, null, classStr);
        }
        _db.add(CLASS_HEAP, null, WalaRepresentation.classConstant(c), classStr);
        ...
        if(c instanceof ShrikeClass) { //We have currently disabled annotations for Android
            ...
            if (annotations != null) {
                for (Annotation annotation : annotations) {
                    _db.add(CLASS_ANNOTATION, null, classStr, fixTypeString(annotation.getType().toString()));
                }
            }
        }
    }
    ...
    void writeDirectSuperclass(IClass sub, IClass sup) {
        _db.add(DIRECT_SUPER_CLASS, null, writeType(sub.getReference()), writeType(sup.getReference()));
    }

    void writeDirectSuperinterface(IClass clazz, IClass iface) {
        _db.add(DIRECT_SUPER_IFACE, null, writeType(clazz.getReference()), writeType(iface.getReference()));
    }
    ...
    private String writeType(TypeReference t) {
        ...
        if (t.isArrayType() && inMap == null) {
            ...
            writeArrayTypes(typeName, writeType(componentType), null);
            _db.add(CLASS_HEAP, null, classConstant(typeName), typeName);
        }
    ...
    private void writePhantomMethod(MethodReference m) {
        ...
        if(_phantomMethod.get(sig) == null) {
            ...
            writePhantomMethod(sig, null);
            _db.add(STRING_RAW, null, sig, sig);
            String arity = Integer.toString(m.getNumberOfParameters());
            _db.add(METHOD, null, sig, WalaRepresentation.simpleName(m), WalaRepresentation.params(m), writeType(m.getDeclaringClass()), writeType(m.getReturnType()), m.getDescriptor().toUnicodeString(), arity);
        }
    }
    ...
    void writePhantomBasedMethod(MethodReference m) {
        ...
        if(_phantomBasedMethod.get(sig) == null) {
            ...
            _db.add(PHANTOM_BASED_METHOD, null, sig);
        }
...
//        _db.add(METHOD, sig, _rep.simpleName(m), _rep.params(m), writeType(m.getDeclaringClass()), writeType(m.getReturnType()), m.getDescriptor().toUnicodeString());
    ...
    void writeEnterMonitor(IMethod m, SSAMonitorInstruction instruction, Local var, Session session) {
        ...
        _db.add(ENTER_MONITOR, null, insn, str(index), _rep.local(m, var), methodId);
    }
    ...
    void writeExitMonitor(IMethod m, SSAMonitorInstruction instruction, Local var, Session session) {
        ...
        _db.add(EXIT_MONITOR, null, insn, str(index), _rep.local(m, var), methodId);
    }
    ...
    void writeAssignLocal(IMethod m, SSAInstruction instruction, Local to, Local from, Session session) {
        ...
        String insn = _rep.instruction(m, instruction, index);
        String methodId = _rep.signature(m);
        writeAssignLocal(insn, index, _rep.local(m, from), _rep.local(m, to), methodId, null);
    }
    ...
    void writeAssignHeapAllocation(IR ir, IMethod m, SSANewInstruction instruction, Local l, Session session) {
        ...
        _db.add(NORMAL_HEAP, null, heap, writeType(instruction.getConcreteType()));
        ...
        String insn = _rep.instruction(m, instruction, index);
        String methodId = _rep.signature(m);
        _db.add(ASSIGN_HEAP_ALLOC, null, insn, str(index), heap, _rep.local(m, l), methodId, ""+getLineNumberFromInstruction(ir, instruction));
    }
    ...
    private void writeAssignNewMultiArrayExprHelper(IR ir, IMethod m, SSANewInstruction instruction, Local l, String assignTo, TypeReference arrayType, Session session) {
        ...
        _db.add(NORMAL_HEAP, null, heap, writeType(arrayType));
        _db.add(ASSIGN_HEAP_ALLOC, null, insn, str(index), heap, assignTo, methodId, ""+getLineNumberFromInstruction(ir, instruction));
        ...
        if (componentType.isArrayType()) {
            ...
            _db.add(STORE_ARRAY_INDEX, null, storeInsn, str(storeInsnIndex), childAssignTo, assignTo, methodId);
            writeLocal(childAssignTo, writeType(componentType), methodId, null);
        }
    }
    ...
    */
    ...
    private void writeAssignStringConstant(IR ir, IMethod m, SSAInstruction instruction, Local l, ConstantValue s, Session session) {
        ...
        String heapId = writeStringConstant(constant, null);
        ...
        _db.add(ASSIGN_HEAP_ALLOC, null, insn, str(index), heapId, _rep.local(m, l), methodId, ""+getLineNumberFromInstruction(ir, instruction));
    }
    ...
    private void writeAssignNull(IMethod m, SSAInstruction instruction, Local l, Session session) {
        ...
        _db.add(ASSIGN_NULL, null, insn, str(index), _rep.local(m, l), methodId);
    }
    ...
    private void writeAssignNumConstant(IMethod m, SSAInstruction instruction, Local l, ConstantValue constant, Session session) {
        ...
        _db.add(ASSIGN_NUM_CONST, null, insn, str(index), constant.toString().substring(1), _rep.local(m, l), methodId);
    }
    ...
    private void writeAssignMethodTypeConstant(IMethod m, SSAInstruction instr, Local l, ConstantValue constant, Session session) {
        ...
        writeMethodTypeConstant(heap, null);
        _db.add(ASSIGN_HEAP_ALLOC, null, insn, str(index), heap, _rep.local(m, l), methodId, "0");
    }
    ...
    private void writeAssignMethodHandleConstant(IMethod m, SSAInstruction instr, Local l, ConstantValue constant, Session session) {
        ...
        writeMethodHandleConstant(heap, handleName, null);
        _db.add(ASSIGN_HEAP_ALLOC, null, insn, str(index), heap, _rep.local(m, l), methodId, "0");
    }
    ...
    private void writeAssignClassConstant(IMethod m, SSAInstruction instruction, Local l, ConstantValue constant, Session session) {
        ...
        _db.add(CLASS_HEAP, null, heap, actualType);
        int index = session.calcInstructionNumber(instruction);
        String insn = _rep.signature(m) + "/assign/instruction" + index; // Not using _rep.instruction() because we do not want to be identified by our instr
        String methodId = _rep.signature(m);
    ...
    void writeAssignCast(IMethod m, SSAInstruction instruction, Local to, Local from, TypeReference t, Session session) {
        ...
        _db.add(ASSIGN_CAST, null, insn, str(index), _rep.local(m, from), _rep.local(m, to), writeType(t), methodId);
    }
    ...
    void writeAssignCastNumericConstant(IMethod m, SSAInstruction instruction, Local to, Local from, TypeReference t, Session session) {
        ...
        _db.add(ASSIGN_CAST_NUM_CONST, null, insn, str(index), from.getValue(), _rep.local(m, to), writeType(t), methodId);
    }
    ...
    void writeAssignCastNull(IMethod m, SSAInstruction instruction, Local to, TypeReference t, Session session) {
        ...
        _db.add(ASSIGN_CAST_NULL, null, insn, str(index), _rep.local(m, to), writeType(t), methodId);
    }
    ...
    private void writeInstanceField(IMethod m, SSAInstruction instruction, FieldReference f, Local base, Local var, Session session, PredicateFile predicateFile) {
        ...
        _db.add(predicateFile, null, insn, str(index), _rep.local(m, var), _rep.local(m, base), fieldId, methodId);
    }
    ...
    private void writeStaticField(IMethod m, SSAInstruction stmt, FieldReference f, Local var, Session session, PredicateFile predicateFile) {
        ...
        TypeReference declaringClass = getCorrectFieldDeclaringClass(f, m.getClassHierarchy());
        String fieldId = WalaRepresentation.signature(f, declaringClass);
        _db.add(predicateFile, null, insn, str(index), _rep.local(m, var), fieldId, methodId);
    }
    ...
    private void writeLoadOrStoreArrayIndex(IMethod m, SSAInstruction instruction, Local base, Local var, Local arrIndex, Session session, PredicateFile predicateFile) {
        ...
        _db.add(predicateFile, null, insn, str(index), _rep.local(m, var), _rep.local(m, base), methodId);

        if (arrIndex != null)
            _db.add(ARRAY_INSN_INDEX, null, insn, _rep.local(m, arrIndex));
    }

    void writeApplicationClass(IClass application) {
        _db.add(APP_CLASS, null, writeType(application.getReference()));
    }
    ...
    public void writeField(IField f) {
        ...
        _db.add(FIELD_SIGNATURE, null, fieldId, writeType(f.getReference().getDeclaringClass()), WalaRepresentation.simpleName(f), writeType(f.getFieldTypeReference()));
        if(f instanceof FieldImpl) { //Currently annotations do not work on android and are disabled
            ...
            if (annotations != null) {
                for (Annotation annotation : annotations) {
                    _db.add(FIELD_ANNOTATION, null, fieldId, fixTypeString(annotation.getType().toString()));
                }
            }
        }
    }
    ...
    void writeFieldModifier(IField f, String modifier) {
        String fieldId = WalaRepresentation.signature(f);
        _db.add(FIELD_MODIFIER, null, modifier, fieldId);
    }
    ...
    void writeMethodModifier(IMethod m, String modifier) {
        String methodId = _rep.signature(m);
        _db.add(METHOD_MODIFIER, null, modifier, methodId);
    }
    ...
    void writeReturn(IMethod m, SSAInstruction instruction, Local l, Session session) {
        ...
        _db.add(RETURN, null, insn, str(index), _rep.local(m, l), methodId);
    }
    ...
    void writeReturnVoid(IMethod m, SSAInstruction instruction, Session session) {
        ...
        _db.add(RETURN_VOID, null, insn, str(index), methodId);
    }
    ...
    void writeNativeReturnVar(IMethod m) {
        ...
        _db.add(NATIVE_RETURN_VAR, null, var, methodId);
        writeLocal(var, writeType(m.getReturnType()), methodId, null);
    }
    ...
    void writeGoto(IMethod m, SSAGotoInstruction instruction, SSAInstruction to, Session session) {
        ...
        _db.add(GOTO, null, insn, str(index), str(indexTo), methodId);
    }
    ...
    void writeIf(IMethod m, SSAConditionalBranchInstruction instruction, Local var1, Local var2, SSAInstruction to, Session session) {
        ...
        writeIf(insn, index, indexTo, methodId, null);
        writeIfVar(insn, L_OP, _rep.local(m, var1), null);
        writeIfVar(insn, R_OP, _rep.local(m, var2), null);
    }
//
...
//            return;
        ...
        _db.add(LOOKUP_SWITCH, null, insn, str(instrIndex), _rep.local(inMethod, switchVar), methodId);
        ...
        for(int i = 0; i < casesAndLabels.length; i+=2) {
            ...
            _db.add(LOOKUP_SWITCH_TARGET, null, insn, str(tgIndex), str(targetIndex));
        }
        ...
        _db.add(LOOKUP_SWITCH_DEFAULT, null, insn, str(defaultIndex));
    }
    ...
    void writeUnsupported(IMethod m, IR ir, SSAInstruction instruction, Session session) {
        ...
        _db.add(UNSUPPORTED_INSTRUCTION, null, insn, str(index), methodId);
    }
    ...
    void writeThrow(IMethod m, SSAThrowInstruction instruction, Local l, Session session) {
        ...
        _db.add(THROW, null, insn, str(index), _rep.local(m, l), methodId);
    }
    ...
    void writeThrowNull(IMethod m, SSAThrowInstruction instruction, Session session) {
        ...
        _db.add(THROW_NULL, null, insn, str(index), methodId);
    }
    ...
    void writeExceptionHandlerPrevious(IMethod m, SSACFG.ExceptionHandlerBasicBlock current, SSACFG.ExceptionHandlerBasicBlock previous, SessionCounter counter) {
        ...
        _db.add(EXCEPT_HANDLER_PREV, null, _rep.handler(m, currCatch, currType, counter, 0), _rep.handler(m, prevCatch, prevType, counter, prevNumOfScopes));
    }
...
//                        handlerIndex +" ("+ beginIndex + " - " + endIndex + ")");
            while (excTypes.hasNext()) {
                ...
                _db.add(EXCEPTION_HANDLER, null, insn, _rep.signature(m), str(handlerIndex), fixTypeString(excType.getName().toString()), _rep.local(m, caught), str(beginIndex), str(endIndex));
                if (prev != null)
                    _db.add(EXCEPT_HANDLER_PREV, null, insn, prev);
                prev = insn;
            }
        }
    ...
    void writeThisVar(IMethod m) {
        ...
        writeThisVar(methodId, thisVar, type, null);
    }

    void writeMethodDeclaresException(IMethod m, TypeReference exception) {
        writeMethodDeclaresException(_rep.signature(m), writeType(exception), null);
    }
    ...
    void writeFormalParam(IMethod m, int paramIndex, int actualIndex) {
        ...
        writeFormalParam(methodId, var, type, actualIndex, null);
    }

    void writeLocal(IMethod m, Local l) {
        String local = _rep.local(m, l);
        writeLocal(local, writeType(l.getType()), _rep.signature(m), null);
    }
...
//        Local l = new Local(varname,-1, TypeReference.JavaLangInvokeMethodHandle);
    ...
    private void writeActualParams(IMethod inMethod, IR ir, SSAInvokeInstruction instruction, String invokeExprRepr, Session session, TypeInference typeInference) {
        if (instruction.isStatic()) {
            ...
            for (int i = 0; i < instruction.getNumberOfPositionalParameters(); i++) {
                ...
                writeActualParam(i, invokeExprRepr, _rep.local(inMethod, l), null);
            }
        }
        else {
            ...
            for (int i = 1; i < instruction.getNumberOfPositionalParameters(); i++) {
                Local l = createLocal(ir, instruction, instruction.getUse(i), typeInference);
                writeActualParam(i-1, invokeExprRepr, _rep.local(inMethod, l), null);
            }
        ...
        if (instruction instanceof SSAInvokeDynamicInstruction) {
            ...
            for (int j = 0; j < (invokedynamic).getBootstrap().callArgumentCount(); j++) {
                ...
                _db.add(BOOTSTRAP_PARAMETER, null, str(j), invokeExprRepr, _rep.local(inMethod, l));
    ...
    void writeInvoke(IMethod inMethod, IR ir, SSAInvokeInstruction instruction, Local to, Session session, TypeInference typeInference) {
        ...
        if(to != null)
            _db.add(ASSIGN_RETURN_VALUE, null, insn, _rep.local(inMethod, to));
    }
    ...
    private String writeInvokeHelper(IMethod inMethod, IR ir, SSAInvokeInstruction instruction, Session session, TypeInference typeInference) {
        ...
        if(sourceLineNum != -1)
            _db.add(METHOD_INV_LINE, null, insn, str(sourceLineNum));
...
//            MethodReference dynInfo = instruction.getDeclaredTarget();
            ...
            writeInvokedynamic(insn, index, sig, targetRef.getName().toString(), fixTypeString(targetRef.getReturnType().toString()), dynArity, parameterTypes.toString(), -1, methodId, null);
        }
        else if (instruction.isStatic()) {
            _db.add(STATIC_METHOD_INV, null, insn, str(index), _rep.signature(targetRef), methodId);
            ...
            _db.add(VIRTUAL_METHOD_INV, null, insn, str(index), _rep.signature(targetRef), _rep.local(inMethod, l), methodId);
            //_db.add(VIRTUAL_METHOD_INV, insn, _rep.signature(targetRef), methodId);
        }
        else if (instruction.isSpecial()) {
            Local l = createLocal(ir, instruction, instruction.getReceiver(),typeInference);
            _db.add(SPECIAL_METHOD_INV, null, insn, str(index), _rep.signature(targetRef), _rep.local(inMethod, l), methodId);
    ...
    void addMockExceptionThrows(MethodReference mr, Iterable<String> declaredExceptions)
    {
        ...
        for(String declaredExc : declaredExceptions)
        {
            ...
            writeLocal(var, declaredExc, methodSig, null);
            heap = methodSig + "/new " + declaredExc + "/0";
            _db.add(NORMAL_HEAP, null, heap, declaredExc);
            newInstr = methodSig + "/assign/instruction" + str(i);
            _db.add(ASSIGN_HEAP_ALLOC, null, newInstr, str(i), heap, var, methodSig, "0");
            specInvInstr = methodSig +"/" + declaredExc +".<init>/0" ;
            targetRef = "<" + declaredExc + ":  void <init>()>";
            _db.add(SPECIAL_METHOD_INV, null, specInvInstr, str(i+1), targetRef, var, methodSig);
            throwInstr = methodSig + "/throw " +varBase + Integer.toString(i) + "/0";
            _db.add(THROW, null, throwInstr, str(i+2),var, methodSig);
        }
    }
...
//
    void writeAssignComparison(IMethod m, SSAComparisonInstruction instruction, Local left, Local op1, Local op2, Session session) {
        ...
        writeAssignBinop(insn, index, _rep.local(m, left), methodId, null);

        _db.add(ASSIGN_OPER_FROM, null, insn, _rep.local(m, op1));
        _db.add(ASSIGN_OPER_FROM, null, insn, _rep.local(m, op2));
    ...
    void writeAssignUnop(IMethod m, SSAUnaryOpInstruction instruction, Local to, Local from, Session session) {
        ...
        writeAssignUnop(insn, index, _rep.local(m, to), methodId, null);
        _db.add(ASSIGN_OPER_FROM, null, insn, _rep.local(m, from));
    }
    ...
    void writeAssignArrayLength(IMethod m, SSAArrayLengthInstruction instruction, Local to, Local from, Session session) {
        ...
        _db.add(ASSIGN_UNOP, null, insn, str(index), _rep.local(m, to), methodId);
        _db.add(ASSIGN_OPER_FROM, null, insn, _rep.local(m, from));
    }
    ...
    void writeAssignInstanceOf(IMethod m, SSAInstanceofInstruction instruction, Local to, Local from, TypeReference t, Session session) {
        ...
        _db.add(ASSIGN_INSTANCE_OF, null, insn, str(index), _rep.local(m, from), _rep.local(m, to), writeType(t), methodId);
    }
