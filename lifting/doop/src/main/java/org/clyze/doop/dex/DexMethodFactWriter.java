...
class DexMethodFactWriter extends JavaFactWriter {
    ...
    public void writeMethod(Collection<FieldOp> fieldOps,
        ...
        _db.add(METHOD, null, methId, mf.simpleName, mf.paramsSig, mf.declaringClass, mf.retType, mf.jvmSig, mf.arity);
        ...
        for (Annotation annotation : m.getAnnotations()) {
            ...
            if ("dalvik.annotation.Throws".equals(annotType)) {
                for (String exception : getAnnotationValues(annotation, (ev -> ((DexBackedTypeEncodedValue) ev).getValue())))
                    writeMethodDeclaresException(methId, raiseTypeId(exception), null);
            ...
            _db.add(METHOD_ANNOTATION, null, methId, annotType);
        }

        for (AccessFlags f : flags)
            _db.add(METHOD_MODIFIER, null, f.toString(), methId);
        ...
        for (MethodParameter param : m.getParameters()) {
            ...
            writeFormalParam(methId, var, type, i, null);
            for (Annotation annot : param.getAnnotations())
                _db.add(PARAM_ANNOTATION, null, methId, str(i), raiseTypeId(annot.getType()));
            i++;
        }
        ...
        if (!isStatic) {
            ...
            writeThisVar(methId, thisVar, type, null);
        }
        if (isNative && !"void".equals(mf.retType)) {
            String var = JavaRepresentation.nativeReturnVarOfMethod(methId);
            _db.add(NATIVE_RETURN_VAR, null, var, methId);
            writeLocal(var,  mf.retType, methId, null);
        }
    ...
    private void processTryBlocks(Iterable<? extends DexBackedTryBlock> tryBlocks) {
        ...
        for (MoveExceptionInfo mei : exceptionMoves) {
            ...
            for (Handler hi : containingHandlers) {
                try {
                    ...
                    _db.add(EXCEPTION_HANDLER, null, insn, methId, str(handlerIndex), hi.excType, localA, str(startIndex), str(endIndex));
        ...
        for (Map.Entry<Handler, Handler> entry : previousHandlers.entrySet()) {
            ...
            if ((currInsn != null) && (prevInsn != null))
                _db.add(EXCEPT_HANDLER_PREV, null, currInsn, prevInsn);
            else
    ...
    private void resolveAndWriteBranches() {
        addressToIndex.forEach((addr, index) ->
            _db.add(DEX_INSTR_ADDR_MAP, null, methId, str(index), str(addr)));
        ...
        for (RawGoto g : ifs) {
            ...
            if (indexTo == null)
                ...
                writeIf(g.insn, g.index, indexTo, methId, null);
        }
    }
    ...
    private void writeDebugItems(Iterable<? extends DebugItem> debugItems) {
        for (DebugItem di : debugItems) {
            if (di instanceof StartLocal) {
                ...
                writeLocal(local(reg), raiseTypeId(rl.getType()), methId, null);
    ...
    private void writeRegisterType(int reg, String regType) { // , Map<Integer, String> regTypes) {
        ...
        writeLocal(local(reg), regType, methId, null);
    }
    ...
    private void writeIf(Instruction instr, int regL, int regR, Opcode op, int index) {
        ...
        writeIfVar(insn, L_OP, local(regL), null);
        if (regR != -1)
            writeIfVar(insn, R_OP, local(regR), null);

        switch (op) {
        case IF_EQ:  case IF_EQZ:  writeOperatorAt(insn, "==", null); break;
        case IF_NE:  case IF_NEZ:  writeOperatorAt(insn, "!=", null); break;
        case IF_LT:  case IF_LTZ:  writeOperatorAt(insn,  "<", null); break;
        case IF_GE:  case IF_GEZ:  writeOperatorAt(insn, ">=", null); break;
        case IF_GT:  case IF_GTZ:  writeOperatorAt(insn,  ">", null); break;
        case IF_LE:  case IF_LEZ:  writeOperatorAt(insn, "<=", null); break;
        }
    }
    ...
    private void handleArrayPayload(ArrayPayload payload) {
        ...
        try {
            ...
            for (int idx = 0; idx < numbersSize; idx++)
                _db.add(ARRAY_INITIAL_VALUE_FROM_CONST, null, insn, local(regDest), str(idx), numbers.get(idx).toString(), heapId);
        } catch (Exception ex) {
    ...
    private void writeMoveResult(Instruction instr, int index, Opcode op) {
        ...
        switch (objReturnInfo.op) {
            ...
            case INVOKE_SUPER_RANGE: {
                ...
                _db.add(ASSIGN_RETURN_VALUE, null, insn, local(regDest));
                // Sanity check.
    ...
    private void writeInitialArrayValues(String insn, int regDest,
        ...
        for (int idx = 0; idx < argRegs.length; idx++)
            _db.add(ARRAY_INITIAL_VALUE_FROM_LOCAL, null, insn, local(regDest), str(idx), local(argRegs[idx]), heap[0]);
    }
    ...
    private void writeSwitchTargets(Instruction instr, PredicateFile predicateFile) {
        ...
        for (SwitchElement elem : ((SwitchPayload) instr).getSwitchElements()) {
            ...
            _db.add(predicateFile, null, insn, str(elem.getKey()), str(branchAddr));
        }
    }
    ...
    private void writeSwitchKey(Instruction instr, int index, PredicateFile predicateFile) {
        ...
        _db.add(predicateFile, null, insn, str(index), local(testRegister), methId);
    ...
    private void writeLoadOrStoreArrayIndex(ThreeRegisterInstruction tri, int index, PredicateFile predicateFile) {
        ...
        String insn = instructionId("assign", index);
        _db.add(predicateFile, null, insn, str(index), local(valReg), local(arrayReg), methId);
        _db.add(ARRAY_INSN_INDEX, null, insn, local(indexReg));
    }
    ...
    private void writeBinopTwoRegPlusLit(TwoRegisterInstruction tri, int index) {
        ...
        String insn = instructionId("assign", index);
        writeAssignBinop(insn, index, local(regDest), methId, null);
        writeAssignOperFrom(insn, L_OP, local(regSource), null);
    }

    private void writeBinopThreeReg(int regDest, int regSource1, int regSource2, int index) {
        String insn = instructionId("assign", index);
        writeAssignBinop(insn, index, local(regDest), methId, null);
        writeAssignOperFrom(insn, L_OP, local(regSource1), null);
        writeAssignOperFrom(insn, R_OP, local(regSource2), null);
    }
    ...
    private void writeAssignCast(OneRegisterInstruction ori, ReferenceInstruction ri, int index) {
        ...
        _db.add(ASSIGN_CAST, null, insn, str(index), local(reg), local(reg), typeName, methId);
    }
    ...
    private void writeAssignInstanceOf(TwoRegisterInstruction tri, ReferenceInstruction ri, int index) {
        ...
        _db.add(ASSIGN_INSTANCE_OF, null, insn, str(index), from, to, className, methId);
    }

    private void writeAssignLocal(TwoRegisterInstruction tri, int index) {
        String to = local(tri.getRegisterA());
        String from = local(tri.getRegisterB());
        writeAssignLocal(instructionId("assign", index), index, from, to, methId, null);
    }
    ...
    private void writeFieldOp(Instruction instr, PredicateFile target,
        ...
        fieldOps.add(new FieldOp(target, insn, str(index), localA, localB, fi, methId, null));
    }
    ...
    private String writeArrayTypes(String arrayType) {
        ...
        writeArrayTypes(arrayType, componentType, null);
    ...
    private void writeInvoke(Instruction instr, Opcode op, int index) {
        ...
        if (objReturnInfo.baseReg != null) {
            ...
            writeActualParam(0, insn, base, null);
            argStartPos = 1;
        }
...
//        System.out.println("argRegs = " + objReturnInfo.argRegs);
        ...
        for (int argPos = 0; argPos < argRegs.length; argPos++)
            writeActualParam(argStartPos + argPos, insn, local(argRegs[argPos]), null);

        if (lineNoInteger != null)
            _db.add(METHOD_INV_LINE, null, insn, lineNo);

        switch (op) {
            case INVOKE_DIRECT:
            case INVOKE_DIRECT_RANGE:
                _db.add(SPECIAL_METHOD_INV, null, insn, str(index), mSig.sig, base, methId);
                break;
            case INVOKE_STATIC:
            case INVOKE_STATIC_RANGE:
                _db.add(STATIC_METHOD_INV, null, insn, str(index), mSig.sig, methId);
                break;
            case INVOKE_VIRTUAL:
            case INVOKE_VIRTUAL_RANGE:
            case INVOKE_INTERFACE:
            case INVOKE_INTERFACE_RANGE:
                _db.add(VIRTUAL_METHOD_INV, null, insn, str(index), mSig.sig, base, methId);
                break;
            case INVOKE_SUPER:
            case INVOKE_SUPER_RANGE:
                _db.add(SUPER_METHOD_INV, null, insn, str(index), mSig.sig, base, methId);
                break;
            default:
    ...
    private void writeAssignStringConstant(int reg, String content, int index) {
        String heapId = writeStringConstant(content, null);
        String insn = instructionId("assign", index);
        String lineNo = strOfLineNo(findLineForInstructionIndex(index));
        String var = local(reg);
        _db.add(ASSIGN_HEAP_ALLOC, null, insn, str(index), heapId, var, methId, lineNo);

        if (extractRegisterTypes)
            writeLocal(var, "java.lang.String", methId, null);
    }
    ...
    private void writeAssignHeapAllocation(int reg, String type, int index,
        ...
        _db.add(NORMAL_HEAP, null, heap, type);
        String lineNo = strOfLineNo(findLineForInstructionIndex(index));
        String var = local(reg);
        _db.add(ASSIGN_HEAP_ALLOC, null, insn, str(index), heap, var, methId, lineNo);

        if (isEmptyArray)
            _db.add(EMPTY_ARRAY, null, heap);

        if (extractRegisterTypes)
            writeLocal(var, type, methId, null);
    }
    ...
    private void writeAssignNumConstant(int reg, String constant, int index) {
        String insn = instructionId("assign", index);
        _db.add(ASSIGN_NUM_CONST, null, insn, str(index), constant, local(reg), methId);
    }
