...
class FactGenerator implements Runnable {
    ...
    PresenceCondition getFeatures(String className, Host h) {
        ...
        return _annotations.getPC(className, getPosition(h));
    }

    String getFactPC(String clsName, Host hd, Host... tl) {
        String pc = getFeatures(clsName, hd).toString();
        for (Host h: tl) {
            String hpc = getFeatures(clsName, h).toString();
            if (!pc.toString().isEmpty() && !hpc.isEmpty()) {
                pc = "(" + pc + ") /\\ (" + hpc + ")";
            } else if (!hpc.isEmpty()) {
                pc = hpc;
            }
    ...
    private boolean isPhantom(Type t, PresenceCondition pc) {
        boolean isPhantom = false;
        if (t instanceof RefLikeType) {
            if (t instanceof RefType)
                isPhantom = ((RefType) t).getSootClass().isPhantom();
            else if (t instanceof ArrayType)
                isPhantom = isPhantom(((ArrayType) t).getElementType(), pc);
        }
        if (isPhantom)
            _writer.writePhantomType(t, pc);
        return isPhantom;
    }
    ...
    private boolean isPhantomBased(SootMethod m) {
        for (SootClass clazz: m.getExceptions()) {
            ...
            PresenceCondition pc = new PresenceCondition(getFactPC(clsName, clazz));
            if (isPhantom(clazz.getType(), pc)) {
                if (_reportPhantoms)
    ...
    private void generatePhantom(Object cause) {
        if (cause instanceof SootClass) {
            String clsName = ((SootClass)cause).getName();
            PresenceCondition pc = new PresenceCondition(getFactPC(clsName, (SootClass) cause));
