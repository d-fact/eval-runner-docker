...
class JimpleListenerImpl extends JimpleBaseListener {
	...
	void enterKlass(KlassContext ctx) {
		...
		klass = new Klass(
				...
				hasToken(ctx, "final"),
				hasToken(ctx, "public"),
				hasToken(ctx, "protected"),
				hasToken(ctx, "private")
		)
	...
	void enterMethod(MethodContext ctx) {
		...
		method = new Method(
				...
				ctx.modifier()?.any { it.text == "synchronized" },
				ctx.modifier()?.any { it.text == "final" },
				ctx.modifier()?.any { it.text == "synthetic" },
				ctx.modifier()?.any { it.text == "public" },
				ctx.modifier()?.any { it.text == "protected" },
				ctx.modifier()?.any { it.text == "private" },
