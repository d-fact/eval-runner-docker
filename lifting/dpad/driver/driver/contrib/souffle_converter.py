#!/usr/bin/env python3
"""
This program is part of vmware/differential-datalog project.
...
class SouffleConverter(object):
    ...
    def process_input(self, inputdecl, user_def_path):
        ...
        for r in rels:
            ...
            data = None
            facts_dirs = [user_def_path + "/"] if user_def_path else ["./", "./facts/"]
            for directory in facts_dirs:
                for suffix in ["", ".gz"]:
    ...
    def process_decl(self, decl, input_facts):
        ...
        if fact_decl is not None:
            ...
            return
        if self.process_rel_decl(decl, input_facts):
            return
    ...
    def process_rel_decl(self, decl, user_def_path):
        ...
        if inputdecl is not None:
            self.process_input(inputdecl, user_def_path)
            return True
    ...
    def process_only_facts(self, decl, user_def_path):
        typedecl = getOptField(decl, "TypeDecl")
        if typedecl is not None:
            self.process_type(typedecl)
            return
        self.process_rel_decl(decl, user_def_path)

    def process(self, tree, input_facts):
        decls = getListField(tree, "Declaration", "DeclarationList")
        for decl in decls:
            if self.conversion_options.skip_logic:
                self.process_only_facts(decl, input_facts)
            else:
                self.process_decl(decl, input_facts)
        self.files.output(self.aggregates)
...
def convert(inputName, outputPrefix, options, debug=False):
    ...
    converter.process(tree, options.input_facts)
...
def main():
    ...
    argParser.add_argument("--facts", help="User defined path to input facts")
    ...
    options.input_facts = args.facts
